"use client";

import TypewriterComponent from "typewriter-effect";
import Link from "next/link";
import { useAuth } from "@clerk/nextjs";

import { Button } from "@/components/ui/button";

export const LandingHero = () => {
  const { isSignedIn } = useAuth();

  return (
    <div className="text-white font-bold py-36 text-center space-y-5">
      <div className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl space-y-5 font-extrabold">
        <h1>The Ultimate Hub for</h1>
        <div className="text-transparent bg-clip-text bg-gradient-to-r from-gray-400 to-gray-600">
          <TypewriterComponent
            options={{
              strings: [
                "Chatbot Assistance.",
                "Photo Generation.",
                "Blog Writing.",
                "Mail Composition.",
                "Video Creation.",
                "Music Generation.",
                "Code Writing.",
                "Creative Inspiration.",
              ],
              autoStart: true,
              loop: true,
            }}
          />
        </div>
      </div>
      <div className="text-sm md:text-xl font-light text-gray-400">
        Elevate your creativity with the power of AI.
      </div>
      <div>
        <Link href={isSignedIn ? "/dashboard" : "/sign-up"}>
          <Button variant="home" className="md:text-lg p-4 md:p-6 rounded-full font-semibold">
            Start Your AI Journey
          </Button>
        </Link>
      </div>
      <div className="text-gray-400 text-xs md:text-sm font-normal">
        Embrace AI innovation for free.
      </div>
    </div>

  );
};
